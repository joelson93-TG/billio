...
// Props reçues du Server Component (app/page.tsx)
interface LandingPageClientProps {
  initialPricing: PricingData;
  initialTutorial: TutorialData | null;
...
function getMonthlyEntrepreneurCount(): number {
  ...
  const variation = (monthIndex % 12) * 45;
...
/* =========================================================
   ICÔNES SVG — inchangées (voir version précédente)
   ========================================================= */
...
/* =========================================================
   MODALE réutilisable — avec fermeture Escape + aria
   ========================================================= */
function Modal({
  isOpen,
  onClose,
  ...
  return (
    ...
    >
      ...
      >
        ...
        <h3 id="modal-title" className="text-2xl md:text-3xl font-bold text-[#070235] mb-6 pr-8">
          {title}
        </h3>
...
export default function LandingPageClient({
  ...
  // Initialisés avec les vraies données venant du serveur (SEO + pas de FOUC)
  const [pricing, setPricing] = useState<PricingData>(initialPricing);
  const [tutorial, setTutorial] = useState<TutorialData | null>(initialTutorial);
  const [dashboardImages, setDashboardImages] = useState<string[]>(
    initialScreenshots.length > 0 ? initialScreenshots : [DEFAULT_DASHBOARD_IMAGE]
  );
  ...
  // Firestore : écoute en temps réel du pricing (prend le relais après le SSR)
  useEffect(() => {
  ...
  // Firestore : écoute en temps réel du tutoriel
  useEffect(() => {
  ...
  // Firestore : écoute en temps réel des captures d'écran
  useEffect(() => {
      ...
      <section className="pt-40 pb-20 px-6 md:px-20 relative overflow-hidden flex flex-col items-center text-center bg-white min-h-screen">
        ...
        <div className="absolute top-1/4 right-[5%] float-anim opacity-80 hidden md:block z-20">
          <div className="bg-white p-2 rounded-xl shadow-[0_25px_50px_-12px_rgba(0,0,0,0.15)] transform rotate-6 border border-[#e0e3e5] w-48 overflow-hidden">
            <img
              alt="Modèle de facture professionnelle Billio"
              ...
              <div className="bg-[#1e1b4b] rounded-3xl p-6 flex flex-col justify-end shadow-lg row-span-2 relative">
                <div className="absolute inset-0 z-0">
                  <img
                    ...
                    alt="Facture conforme aux normes OHADA"
                    loading="lazy"
